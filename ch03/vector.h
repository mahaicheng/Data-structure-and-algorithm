...
class vector {
 public:
  ...
  explicit vector(int initSize = 0)
      : theSize(initSize), theCapacity(theSize + 1) {
  ...
  const Object& operator[](int pos) const { return objects[pos]; }

  Object& operator[](int pos) { return objects[pos]; }
  ...
  void push_back(const Object& item) {
    if (theSize == theCapacity) reserve(theCapacity * 2);
