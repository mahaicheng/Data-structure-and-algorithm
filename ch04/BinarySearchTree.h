...
class BinarySearchTree {
 public:
  ...
  BinarySearchTree(const BinarySearchTree& rhs) { root = clone(rhs); }
  ...
  bool contains(const T& item) const { return contains(item, root); }
 ...
 private:
  ...
  void insert(const T& item, BinaryNode*& root) const {
    ...
    if (root->element == item) return;  // no duplicate
